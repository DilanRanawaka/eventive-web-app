...
const { Package, validate } = require("../models/packages");
...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    imageName = Date.now() + new Date().getTime() + ".jpg";
    cb(null, imageName);
  },
});
...
router
  ...
  .post(upload.single("image"), async (req, resp) => {
    try {
      await Package.find({ _id: req.body.id }).update({
        name: req.body.name,
        description: req.body.description,
        price: req.body.price,
        quantity: req.body.quantity,
...
router.post("/delete", async (req, resp) => {
  try {
    await Package.find({ _id: req.body.id }).remove();
...
router.post("/active/list", async (req, resp) => {
  try {
    const data = await Package.find({ status: 1, quantity: { $gte: 1 } });
    return resp.status(200).send(data);
  } catch (error) {
    return resp.status(500).send({ message: error.message });
  }
